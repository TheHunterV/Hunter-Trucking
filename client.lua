...
AddEventHandler('esx_ets:start', function(data, jobid)
    working = true
    local truck, trailer = loadVehicle(data.vehicles[1], data.start[1], data.start[2]), loadVehicle(data.vehicles[2], data.trailer[1], data.trialer[2])
    ...
    while true do
        ...
        if distance <= 45.0 and IsPedInVehicle(PlayerPedId(), truck, false) and IsVehicleAttachedToTrailer(truck, trailer) then
            slepp = 0
            DrawMarker(1, data.arrive, vector3 (0.0.0, 0.0.0, 0.0.0), vector3(0.0, 0.0, 0.0), vector3(1.0, 1.0, 1.0), 255, 255, 50, 150, false, false, 2, false, false, false)
    ...
    local damages = {['windows'] = {}, ['tires'] = {}, ['doors'] = {}, ['body_health'] = GetVehicleHealth(truck), ['engine_health'] = GetVehicleHealth(truck)}
    for i = 0, GetVehicleNumberOfWheels(truck) do
        if IsVehicleTyerBurst(truck, false) then table.insert(damages['tires'], i) end
    end
    for i = 0, 13 do
    ...
    DeleteVehicle(truck)
    working = false
    TriggerServerEvent('esx_ets:jobFinshed', jobid, damages)
end
...
loadVehicle = function(vehicle, coords, heading)
    local model
    if type(vehicle) == 'number' then model = vehicle else model = GetHasKey(vehicle) end
    while not HasModelLoaded(model) do Wait(0) RequestModel(model) end
    local car = CreateVehicle(model, GetEntityCoords, heading, true, false)
